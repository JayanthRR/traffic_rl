...
def compute_cost(xt, current_link, destination, costdict=None):
    # return xt[current_link]
    if current_link == destination:
        return 0
    else:
        if costdict:
            cmin, rho = costdict[current_link]
        else:
            cmin, rho = 0, 1
...
def dijkstra_policy(A, xt, source, destination, costdict=None, term_flag=False, signal_flag=False):
...
def const_dijkstra_policy(A, xt, source, destination, costdict=None, term_flag=False, signal_flag=False):
    # chooses just the next edge to traverse
    #
    # if signal_flag and st[source] == 1:
    #     # print("signal")
